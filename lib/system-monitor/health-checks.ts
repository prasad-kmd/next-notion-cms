...
export async function checkNotionHealth() {
    ...
    try {
        ...
        return {
            ...
            databases_connected: Object.values(DATABASE_IDS).filter(Boolean).length,
            last_sync: null,
            error_message: (error as { message: string }).message || 'Unknown Notion error'
        };
    }
}
...
export async function checkSupabaseHealth() {
    ...
    try {
        ...
        try {
            ...
            dbSize = Number((sizeResult as unknown as { pg_database_size: number }[])[0]?.pg_database_size || 0);
        } catch (e) {
        ...
        try {
            ...
            activeConnections = Number((connResult as unknown as { count: number }[])[0]?.count || 0);
        } catch (e) {
        ...
        return {
            ...
            error_message: (error as { message: string }).message || 'Unknown Supabase error'
        };
    }
}
...
export async function checkPostHogHealth() {
    ...
    try {
        ...
        return {
            status: 'error' as SystemStatus,
            latency_ms: Math.round(latency),
            error_message: (error as { message: string }).message || 'Unknown PostHog error'
        };
    }
}
