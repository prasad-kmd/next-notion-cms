...
export function CountryTrendChart({ timeRange }: CountryTrendChartProps) {
    ...
    async function fetchData() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setData(transformed);
          setCountries(topSeries.map((s: any) => s.breakdown_value || "Other"));
        }
      } catch (err) {
        console.error(err);
      } finally {
  ...
  if (loading) return <div className="h-64 flex items-center justify-center animate-pulse bg-muted/10 rounded-3xl">Loading country trends...</div>;
