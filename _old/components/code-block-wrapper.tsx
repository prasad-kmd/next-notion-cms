...
export function CodeBlockWrapper({ children }: CodeBlockWrapperProps) {
  ...
  useEffect(() => {
    ...
    const quizPlaceholders = containerRef.current.querySelectorAll(".interactive-quiz-placeholder")
    quizPlaceholders.forEach((placeholder) => {
      if (placeholder.querySelector(".quiz-container")) return
      
      const payload = placeholder.getAttribute("data-quiz-payload")
      if (!payload) return

      try {
        const jsonContent = atob(payload)
        const quizData = JSON.parse(jsonContent)
        
        const quizContainer = document.createElement("div")
        quizContainer.className = "quiz-container"
        placeholder.appendChild(quizContainer)

        const root = createRoot(quizContainer)
        root.render(<Quiz title={quizData.title} questions={quizData.questions} />)
      } catch (e) {
