      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-8 grid gap-4 md:grid-cols-4">
          <div className="md:col-span-2 relative">
            ...
            <Input
              ...
              className="pl-10 h-11 bg-card/50 backdrop-blur-sm border-border"
              value={searchTerm}
          ...
          <div className="relative">
            ...
            <select
              className="flex h-11 w-full rounded-md border border-border bg-card/50 backdrop-blur-sm pl-10 pr-3 py-2 text-sm ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 appearance-none text-foreground"
              value={selectedStore}
          ...
          <Button onClick={() => fetchDeals(searchTerm, selectedStore)} disabled={loading} className="h-11 shadow-lg shadow-primary/20">
              ...
              <Card key={deal.dealID} className="group flex flex-col overflow-hidden border-border bg-card/40 backdrop-blur-md transition-all hover:border-primary/50 hover:shadow-xl hover:shadow-primary/5 hover:-translate-y-1">
                <div className="relative aspect-[16/9] overflow-hidden bg-muted">
                  ...
                  <div className="absolute top-2 right-2 flex flex-col gap-1 items-end">
                    <span className="rounded-full bg-primary px-2 py-1 text-[10px] font-bold text-primary-foreground shadow-lg">
                      -{Math.round(parseFloat(deal.savings))}%
                    </span>
                    {parseFloat(deal.metacriticScore) > 0 && (
                       <span className="rounded-full bg-black/60 backdrop-blur-md px-2 py-1 text-[10px] font-bold text-white border border-white/10">
                ...
                <CardHeader className="p-4 flex-1">
                  <CardTitle className="line-clamp-2 text-base font-bold font-sans group-hover:text-primary transition-colors min-h-[3rem]">{deal.title}</CardTitle>
                </CardHeader>
                <CardContent className="p-4 pt-0">
                  <div className="flex items-end justify-between">
                    ...
                    <div className="text-right">
                       <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-bold">Deal Rating</p>
                       <div className="flex items-center gap-1 justify-end">
                          <span className="text-lg font-mono font-bold text-primary">{deal.dealRating}</span>
                       </div>
                    </div>
                  </div>
                </CardContent>
        ...
        {!loading && deals.length === 0 && (
          <div className="flex h-96 flex-col items-center justify-center text-center rounded-2xl border border-dashed border-border bg-card/20 backdrop-blur-sm">
            ...
            <h3 className="text-xl font-bold font-sans mb-1">No deals found</h3>
